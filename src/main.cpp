...
//     publicar o suscribirse a un canal para enviar y recivir mensajes       //
//     respectivamente                                                        //
//                                                                            //
...
//////////// Indicamos las librerias necesarias para el programa ///////////////
...
char msg[50];                     //Variable donde se se escribira el mensaje
/******************************************************************************/
...
void setup(){                  //Declaramos la función de configuración "setup"
  ...
  Serial.begin(115200);        //La elocidad de la comunicación serial a 115200
  ...
  client.setCallback(callback);  //Indicamos la funcion para mensajes recividos
}
/******************************************************************************/
...
void setup_wifi(){        //Definimos lo que hará la función "setup_wifi"
  ...
  while(WiFi.status()!=WL_CONNECTED){ //Ciclo hasta establecer la conexión
    ...
    Serial.print(".");                //Se envia un caracter cada ciclo
...
void callback(char* topic, byte* payload, unsigned int length){
  Serial.print("Mensaje recivido ["); //Indicamos que hemos recivido un mensaje
  Serial.print(topic);                //y en que canal fue recibido
  Serial.print("]");
  for(int i =0; i < length;i++){      //Escribimos el mensaje caracter por
    Serial.print((char)payload[i]);   //caracter
  }
}
/******************************************************************************/
...
void reconnect(){
  while(!client.connected()){    //Ciclo si no hay conexión con el servidor MQTT
    ...
    if(client.connect("Nombre_Asignado_AlModulo")){ //Si el cliente ya se ha conectado (Nota:El modulo ESP8266 es el cliente)
      Serial.println("conectado!");            //Indicamos que ya esta conectado
    ...
    }else {                                        //Si aun no hay conexión MQTT
      ...
      delay(5000);                        //realizara un nuevo intento en 5 seg.
    }
  }
}
/******************************************************************************/
...
void loop(){
  if(!client.connected()){    //Si no esta conectado el cliente MQTT
    reconnect();              //Se llama a la funcion "reconnect()"
  }
  client.loop();              //Funcion para que el ESP8266 sea cliente MQTT
  long now = millis();        //Guardamos el momento actual en la variable now
  if(now-lastMsg>2000){       //Si el ultimo mensaje fue hace mas de 2 seg.
    lastMsg=now;              //Actualizamos el tiempo para el ultimo mensaje
